...
for i in range(generations): # do it n (generation) times
    ...
    for j in range(int(population/2)): # population/2 because each gives 2 parents
        ...
        obj_val_mutated_child_1 = svm_hp_opt.objective_value(x=x,y=y, chromosome=mutated_child_1, kfold=kfold)[2]
        obj_val_mutated_child_2 = svm_hp_opt.objective_value(x=x,y=y, chromosome=mutated_child_2, kfold=kfold)[2]
        
        
        # for each mutated child, put its obj val next to it
        mutant_1_with_obj_val = np.hstack((obj_val_mutated_child_1, mutated_child_1)) # lines 132 and 140
        
        mutant_2_with_obj_val = np.hstack((obj_val_mutated_child_2, mutated_child_2)) # lines 134 and 143
